    ...
    def get_path_distance(self, path):
        ...
        for k, idx in enumerate(idxs):
            if k == 0:
                ...
                a_list = path1[idxs[k - 1] + 1:idx + 1]
            lists.append(a_list)
        for p in lists:
            for i in range(len(p)):
                distance += self._get_distance(path1[i], path1[(i + 1) % len(p)])
        return distance
...
class AntColony(object):
    ...
    def reset_ants(self, n):
        ...
        for _ in range(self.m):
            self.ants.append(Ant(randint(1, n - 1), capacity=CAPACITY))
...
def main():
    df_city = pd.read_csv(r'D:\ProgramFiles\PycharmProjects\learnpy\learn_model\data\eil30.csv')
    ...
    best_distance = dis_result[-1][1]
    best_path = dis_result[-1][2]
    best_path.append(0)
